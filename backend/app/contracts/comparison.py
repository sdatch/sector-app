"""
Comparison API contract — normalized outcome schema across risk models (v1.1).

Ported from docs/comparison_contract.py (v1.0) with the v1.1 amendments from
docs/user_model_spec.md §3.4:
  - Allocation gains a `portfolio_id` mode (server resolves to weights).
  - Runtime envelope types (OutcomeEnvelope, ComparisonResource) from
    docs/endpoint_behavior_spec.md are defined here so the whole app shares one
    wire vocabulary.

Design principles (unchanged from v1.0):
  1. Every model MUST populate CommonMetrics and OutcomeDistribution; the
     derivation is declared, never hidden (EstimationMethod).
  2. All models run against the same DataSnapshot + RequestScope, so output
     differences reflect model assumptions, not data drift.
  3. Sector attribution is a first-class common block.
  4. Model-specific detail lives in a discriminated union.

Pydantic v2. Target: FastAPI POST /v1/comparisons
"""
...
class OutcomeStatus(str, Enum):
    """Per-model status machine (endpoint spec §4)."""
...
class ComparisonStatus(str, Enum):
    """Aggregate comparison status (endpoint spec §4)."""

    RUNNING = "running"
    COMPLETE = "complete"  # all outcomes succeeded
    PARTIAL = "partial"  # mixed success/failure
    FAILED = "failed"  # none succeeded
...
class Allocation(BaseModel):
    """The candidate allocation being evaluated. Exactly one mode.

    v1.1: `portfolio_id` is the preferred mode — the orchestrator resolves the
    portfolio's positions to sector weights against the current snapshot's
    prices before building the EvaluationContext, so engines are unaffected.
    """

    portfolio_id: UUID | None = None  # v1.1 — server resolves to weights
    ...
    def exactly_one_mode(self) -> Allocation:
        modes = [
            self.portfolio_id is not None,
            bool(self.sector_weights),
            bool(self.ticker_weights),
        ]
        if sum(modes) != 1:
            raise ValueError(
                "Provide exactly one of: portfolio_id, sector_weights, "
                "ticker_weights"
            )
        # Explicit weight modes must sum to 1.0; portfolio_id is resolved
        # server-side and validated after resolution.
        if self.sector_weights or self.ticker_weights:
            weights = (
...
class OutcomeEnvelope(BaseModel):
    """Per-model status wrapper (endpoint spec §4). `outcome` present for
    provisional/complete; `error` present only for failed."""
...
class ComparisonResource(BaseModel):
    """The observable comparison resource (endpoint spec §2). Returned by
    POST /v1/comparisons and GET /v1/comparisons/{id}."""
